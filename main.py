...
def aws_connect(cfg, certs_path):
    ...
    # Reuse your existing device_id as MQTT client_id if present
    client_id = cfg.get("cumulocity", {}).get("device_id", "fx30-test")
...
def publish_measurement(client, topic, series, value, unit):
    aws_publish_json(
        client,
        topic,
        {
...
if __name__ == "__main__":
    ...
    # MN Opens and reads a YAML configuration file.
    ...
    try:
        while True:
            ...
            # ---- MODBUS READS (keeps your 13-measurement logic) ----
            try:
                ...
                if len(values) == 13:
                    def push_all():
                        publish_measurement(aws_client, aws_topic, "Turbidity(202)", values[0], "NTU")
                        publish_measurement(aws_client, aws_topic, "Temp_Turb(204)", values[1], "degC")
                        publish_measurement(aws_client, aws_topic, "pH(206)", values[2], "ph")
                        publish_measurement(aws_client, aws_topic, "Temp_pH(208)", values[3], "degC")
                        publish_measurement(aws_client, aws_topic, "Conductivity(210)", values[4], "mS/cm")
                        publish_measurement(aws_client, aws_topic, "TDS(212)", values[5], "mg/L")
                        publish_measurement(aws_client, aws_topic, "Dis_Oxy(214)", values[6], "mg/L")
                        publish_measurement(aws_client, aws_topic, "TSS(216)", values[7], "mg/L")
                        publish_measurement(aws_client, aws_topic, "Velocity(218)", values[8], "m/s")
                        publish_measurement(aws_client, aws_topic, "Level(220)", values[9], "m")
                        publish_measurement(aws_client, aws_topic, "Bat(226)", values[10], "V")
                        publish_measurement(aws_client, aws_topic, "FloatSwitchState(124)", values[11], "")
                        publish_measurement(aws_client, aws_topic, "DoorSwitchState(128)", values[12], "")
                    ...
                    if last_read is not None and values[11] != last_read[11]:
                        ...
                        publish_measurement(
                            aws_client, aws_topic, "FloatSwitchState(124)", values[11], ""
                        )
                        last_read = values
                    ...
                    if last_read is not None and values[12] != last_read[12]:
                        ...
                        publish_measurement(
                            aws_client, aws_topic, "DoorSwitchState(128)", values[12], ""
                        )
                        last_read = values
